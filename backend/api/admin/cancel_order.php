...
try {
    ...
    // 2️⃣ Enterprise Rules: Determine cancel type
    // - Soft Cancel: Before assignment (status <= APPROVED, no shipper)
    // - Operational Cancel: After assignment (status >= ASSIGNED or has shipper)
    $cancelType = "soft";
    ...
    // 4️⃣ Guard: Operational cancel cannot be reopened (but we still allow it)
    // This is logged in order_history for audit
