...
import { useConfirmDeliveryFailed } from "../../hooks/shipper/useConfirmDeliveryFailed";
...
const OrderDetailShipper = () => {
  ...
  const { submit: submitDeliveryFailed, loading: failSubmitting } =
    useConfirmDeliveryFailed();

  // ==========================
  // DELIVERY FAILED MODAL STATE
  // ==========================
  const [showFailModal, setShowFailModal] = useState(false);
  const [failReason, setFailReason] = useState("");
  const [failNote, setFailNote] = useState("");
  const [failImage, setFailImage] = useState(null);
  const [failPreview, setFailPreview] = useState(null);
  ...
  useEffect(() => {
    ...
    const objectUrl = URL.createObjectURL(failImage);
    setFailPreview(objectUrl);
    return () => URL.revokeObjectURL(objectUrl);
  }, [failImage]);
  ...
  const canConfirmPickup =
    checkOrder && checkSender && checkPackage && actualWeight && pickupImage;
    ...
    <Container className="py-3 mb-5" fluid="md">
      ...
      <Card className="mb-3 shadow-sm border-0">
        ...
        <Card.Body>
          <Row>
            ...
            <Col md={6}>
              <h6 className="text-uppercase text-muted fw-bold small mb-3">
                <PersonFill className="text-success me-1" /> Receiver
                Information
              </h6>
          ...
          <Row className="g-3 mb-3">
            ...
            <Col xs={6} md={3}>
              ...
              <strong>
                ...
                {order.length
                  ? `${Number(order.length)}x${Number(order.width)}x${Number(
                      order.height
                    )} cm`
                  : "N/A"}
              </strong>
      ...
      {status === 6 && (
        <Card className="mb-3 shadow-sm border-danger">
          ...
          <Card.Body>
            <Alert variant="danger">
              This order was marked as <strong>Delivery Failed</strong>. Further
              actions will be handled by Agent/Admin.
            </Alert>
      ...
      >
        <Modal.Header>
          <Modal.Title>
            <BoxSeam className="me-2" /> Pickup Checklist
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            <div className="d-flex align-items-start mb-2">
              <Form.Label className="frcheck me-2">
                Verified Order Code
              </Form.Label>
              <Form.Check
                type="checkbox"
