...
export default function Reports() {
  ...
  return (
    <div className="admin-page" id="report-wrapper">
      ...
      <Card className="card-lux p-3 mt-2">
        <h6 className="fw-bold mb-2">Bảng Mapping Dữ liệu (Enterprise-ready)</h6>
        <Table size="sm" responsive className="table-luxury">
          <thead>
            <tr>
              <th>Widget</th>
              <th>Dimensions</th>
              <th>Metrics</th>
              <th>Nguồn / Join Hint</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>Báo cáo 1 — Đơn hàng theo Trạng thái</td>
              <td>date_bucket × status_id</td>
              <td>count(order_id)</td>
              <td>orders + statuses</td>
            </tr>
            <tr>
              <td>Báo cáo 2 — Doanh thu & Đơn hàng</td>
              <td>date_bucket</td>
              <td>sum(total_fee), count(order_id)</td>
              <td>orders (+ invoices nếu tách)</td>
            </tr>
            <tr>
              <td>Báo cáo 3 — Phân bố Dịch vụ</td>
              <td>service_type_id</td>
              <td>count(order_id)</td>
              <td>orders + service_types</td>
            </tr>
            <tr>
              <td>Báo cáo 4 — Phân bố Thanh toán</td>
              <td>payment_method_id</td>
              <td>count(order_id)</td>
              <td>orders + payment_methods</td>
            </tr>
            <tr>
              <td>Bảng 1 — Tuân thủ SLA</td>
              <td>service_type_id × lead_time_bucket</td>
              <td>pct(count delivered by bucket)</td>
              <td>orders + order_history (lead_time)</td>
            </tr>
            <tr>
              <td>Bảng 2 — Chuyển đổi Quy trình</td>
              <td>status sequence</td>
              <td>count + conversion%</td>
              <td>orders + statuses</td>
            </tr>
            <tr>
              <td>Bảng 3 — Tồn đọng theo Thời gian</td>
              <td>status_id × aging_bucket</td>
              <td>count (WIP only)</td>
              <td>orders (WIP) + order_history (age)</td>
            </tr>
            <tr>
              <td>Bảng 4 — Chất lượng Đại lý</td>
              <td>agent_id</td>
              <td>delivered/wip/failed count</td>
              <td>orders + users(agents)</td>
            </tr>
            <tr>
              <td>Bảng 5 — Thời gian giao hàng Shipper</td>
              <td>shipper_id</td>
              <td>min/q1/median/q3/max lead_time</td>
              <td>orders + order_history + users(shippers)</td>
            </tr>
            <tr>
              <td>Bảng 6 — Ma trận Rủi ro Thất bại</td>
              <td>service_type_id × payment_method_id</td>
              <td>failed_rate (%)</td>
              <td>orders + service_types + payment_methods</td>
            </tr>
          </tbody>
        </Table>
      </Card>
