// frontend/src/pages/admin/FeesManagement.jsx
...
/**
 * Fees - Master Data
 * Expected DB fields (example): fees(id, code, name, type, amount)
 * Endpoint: /get_fees.php
...
export default function FeesManagement() {
  ...
  const [lastSource, setLastSource] = useState(apiUrl("/get_fees.php"));
  ...
  const fetchData = async () => {
    ...
    const endpoint = apiUrl("/get_fees.php");
    ...
    try {
      ...
      setErrorMsg(
        err?.message ||
          "Unable to load fees. Please verify backend URL, CORS, and that the endpoint exists."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      >
        <div>
          <h2 style={{ margin: 0, fontSize: 26 }}>Fees</h2>
          <div style={{ opacity: 0.75, marginTop: 6 }}>
            Master fee rules used to calculate total shipping cost.
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <input
            ...
            placeholder="Search by code, name, type, or amount..."
            style={{
              width: 360,
              maxWidth: "70vw",
              padding: "10px 12px",
      ...
      >
        ...
        {loading ? (
          ...
          <div style={{ padding: 18, opacity: 0.75 }}>No fees found.</div>
        ) : (
              ...
              <tbody>
                {filtered.map((x) => (
                  <tr key={x.id ?? `${x.code}-${x.name}`}>
                    ...
                    <td style={{ padding: 12, borderBottom: "1px solid #f3f3f3" }}>
                      <span style={{ fontWeight: 800 }}>{formatAmount(x.amount)}</span>
                    </td>
